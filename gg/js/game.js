...
async function submitAuth(event) {
    ...
    try {
        const data = await authRequest(`/api/${authMode}.php`, { username, password });
...
async function logoutAuth() {
    try { await authRequest('/api/logout.php', {}); } catch (e) {}
    location.reload();
}
...
async function syncBalanceToServer(immediate = false) {
    ...
    const sendNow = () => fetch('/api/balance.php', {
        method: 'POST',
...
function syncBalanceBeforeUnload() {
    ...
    if (navigator.sendBeacon) {
        navigator.sendBeacon('/api/balance.php', new Blob([payload], { type: 'application/json' }));
    }
}

// Balance is now committed by /api/round on the server; no client-side balance beacon.


async function requestServerRound(betsSnapshot) {
    const res = await fetch('/api/round.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ bets: betsSnapshot }),
    });
    const data = await res.json().catch(() => ({}));
...
function serverRoundTimestamp() {
    const now = new Date();
    return {
        dateStr: new Intl.DateTimeFormat('id-ID', { weekday:'long', day:'numeric', month:'long', year:'numeric' }).format(now),
        timeStr: new Intl.DateTimeFormat('id-ID', { hour:'2-digit', minute:'2-digit', second:'2-digit' }).format(now).replace(/:/g, '.'),
    };
}

async function serverSideDealGame() {
    if (isPlaying) return;
    if (!currentAuthUser) {
        document.getElementById('auth-overlay')?.classList.add('show');
        return;
    ...
    let response;
    try {
        response = await requestServerRound(betsSnapshot);
    } catch (err) {
        isPlaying = false;
        displayBalance = null;
        setStatus(err.message || 'ROUND ERROR', '#b91c1c', 'white');
        try {
            const me = await authRequest('/api/me.php');
            balance = Number(me.user.balance || 0);
    ...
    const r = response.round;
    const {
        pCard1, pCard2, pCard3,
        bCard1, bCard2, bCard3,
        pScore, bScore,
        pMultTotal, bMultTotal,
        winner, lightningCards, payout,
    } = r;

    const baseBet = Number(response.betCost || 0);
    displayBalance = Number(response.balanceAfterCost || 0);
    balance = Number(response.finalBalance || response.user?.balance || 0);
    if (response.user) currentAuthUser = response.user;

    const { dateStr, timeStr } = serverRoundTimestamp();
    if (baseBet > 0) {
        gameHistory.unshift({ dateStr, timeStr, bet: baseBet, net: Number(payout || 0) - baseBet });
    }
    ...
    if (pCard3) {
        ...
        pVis *= await dealCard(pCard3, 'p-cards', 2);
        showMult('p-mult', pVis);
        showScore('p-score', pScore);
    }
    if (bCard3) {
        await sleep(500);
        bVis *= await dealCard(bCard3, 'b-cards', 2);
        showMult('b-mult', bVis);
        showScore('b-score', bScore);
    }
    ...
    renderRoadmap();
    renderHistory();
    resolveBets(winner, Number(payout || 0));
}

// Replace the browser RNG/math deal with server-side round calculation.
dealGame = serverSideDealGame;

// Wrap mutating functions to auto-save after each call
const _origTopUp = topUpBalance; topUpBalance = function ()  { _origTopUp();  saveGameData(); };
const _origClear = clearBets;   clearBets    = function ()  { _origClear();  saveGameData(); };
const _origPlace = placeBet;    placeBet     = function (z) { _origPlace(z); saveGameData(); };
const _origUndo  = undoBet;     undoBet      = function (z) { _origUndo(z);  saveGameData(); };
...
(async function initAuth() {
    try {
        const data = await authRequest('/api/me.php');
        startAuthenticatedGame(data.user);
    } catch (e) {
        document.getElementById('auth-overlay')?.classList.add('show');
        setAuthMode('login');
